...
def fetch_info(conf):
    ...
    try:
        ...
        log_verbose('Connected to Redis at %s:%s' %
                    (conf['host'], conf['port']))
...
def parse_info(info_lines):
    ...
    info["changes_since_last_save"] = info.get(
...
def configure_callback(conf):
    ...
    for node in conf.children:
        ...
        if key == 'host':
            ...
            collectd.warning(
                'redis_info plugin: Unknown config key: %s.' % key)
            continue

    log_verbose('Configured with host=%s, port=%s, instance name=%s, using_auth=%s' % (
        host, port, instance, auth != None))

    CONFIGS.append({'host': host, 'port': port,
...
def dispatch_value(info, key, type, plugin_instance=None, type_instance=None):
    ...
    if plugin_instance is None:
        ...
        collectd.error(
            'redis_info plugin: plugin_instance is not set, Info key: %s' % key)
...
def get_metrics(conf):
    ...
    if plugin_instance is None:
        plugin_instance = '{host}:{port}'.format(
    ...
    for keyTuple, val in REDIS_INFO.iteritems():
        ...
        if key == 'total_connections_received' and val == 'counter':
            dispatch_value(info, 'total_connections_received',
                           'counter', plugin_instance, 'connections_received')
        elif key == 'total_commands_processed' and val == 'counter':
            dispatch_value(info, 'total_commands_processed',
                           'counter', plugin_instance, 'commands_processed')
        else:
